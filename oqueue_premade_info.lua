...
function oq.check_for_wipe()
    if
        (not oq.iam_raid_leader()) or (oq._inside_instance == nil) or
            ((oq._instance_type ~= 'party') and (oq._instance_type ~= 'raid'))
     then
        return
    end
    ...
    elseif (raid_type == OQ.TYPE_ARENA) then
        pdata =
            ...
            oq.encode_mime64_1digit(oq.best_arena_rank(2)) ..
                oq.encode_mime64_1digit(oq.best_arena_rank(3)) .. oq.encode_mime64_1digit(oq.best_arena_rank(5))
    else -- type_raid
...
function oq.scan_for_boss(raid_id)
    ...
    if oq.is_raid_boss(raid_id, UnitName('target')) then
        return 'target'
    end
    if oq.is_raid_boss(raid_id, UnitName('focus')) then
        return 'focus'
    end
            ...
            oq.encode_mime64_1digit(nkilled) ..
                oq.encode_mime64_1digit(maxBosses) ..
                    oq.encode_mime64_1digit(boss_id) ..
                        oq.encode_mime64_1digit(floor(hp / 2)) .. oq.get_raid_boss_progression(name)
...
function oq.render_raid_status(pdata)
    ...
    elseif (R == 63) then
        ...
        s =
            s ..
            '|cff' ..
                (OQ.difficulty_color[D] or '0000ff') ..
                    (OQ.raid_abbrevation[R] or '') .. string.format(OQ.difficulty_abbr[D] or '', n) .. '|r '
        -- progress

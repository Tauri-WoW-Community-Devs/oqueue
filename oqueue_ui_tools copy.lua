...
function oq.CreateFrame(type, name, parent, template)
    ...
    -- print(type, name, parent, template)
    local f = AceGUI:Create(type)
    parent:AddChild(f)
    return f
end
...
function oq.checkbox(parent, x, y, cx, cy, text_cx, text, is_checked, on_click_func)
    local button = oq.CreateFrame('CheckBox', 'OQ_Check', parent, 'UICheckButtonTemplate')
    button:SetWidth(cx)
    button:SetHeight(cy)
    ...
    button:SetCallback('OnClick', on_click_func)
    if (is_checked == 0) then
        is_checked = nil
    end
    button:SetValue(is_checked)
...
function oq.click_label(parent, x, y, cx, cy, text, justify_v, justify_h, font, template)
    return oq.label(parent, 0, 0, cx, cy, text, justify_v, justify_h, font)
end
...
function oq.texture_button(parent, x, y, cx, cy, up_texture, dn_texture, disable_texture, click_func)
    ...
    f:SetCallback('OnClick', click_func)
    -- f:Show()
    return f
end
...
function oq.CreateDropdown(parent, optionTable, isChecked, onClickFunc)
    local dropDown = CreateFrame("Frame", "WPDemoDropDown", parent, "UIDropDownMenuTemplate")

    local onClick = function(self, label, value, checked)
        UIDropDownMenu_SetText(dropDown, label)
        onClickFunc(value)
    end

    local addDropdownItem = function(info, label, value)
        info.text = label
        info.arg1 = label
        info.value = value
        info.arg2 = value
        info.checked = isChecked(value)
        info.isRadio = true
        UIDropDownMenu_AddButton(info)
    end

    local populateFunc = function(info)
        for label, value in tbl.orderedPairs(optionTable) do
            addDropdownItem(info, label, value)
        end
    end
